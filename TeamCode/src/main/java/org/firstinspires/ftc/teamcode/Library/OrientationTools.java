...
public class OrientationTools {
    ...
    public static final double WHEEL_DIAMETER_CMS = 10.0;     // For figuring circumference
    public static final double COUNTS_PER_CM = (COUNTS_PER_MOTOR_REV * DRIVE_GEAR_REDUCTION) / (WHEEL_DIAMETER_CMS * Math.PI);
    ...
    public void driveSidewaysSkystone(OpMode op, double distanceForward, double distanceSideways, double speed, OmniWheel wheel, double startPos, BNO055IMU imu, double smoothness, double smoothnessAdjust, ColorSensor sensor) {
        ...
        while ((wheel.robot.motor_front_right.isBusy() && wheel.robot.motor_front_left.isBusy() && wheel.robot.motor_rear_left.isBusy() && wheel.robot.motor_rear_right.isBusy()) && !colorTools.isSkystoneFound(sensor)) {
    ...
    public void driveSidewaysRedBlue(OpMode op, double distanceForward, double distanceSideways, double speed, OmniWheel wheel, double startPos, BNO055IMU imu, double smoothness, double smoothnessAdjust, ColorSensor sensor) {
        double offset = this.getDegree360(imu) - startPos;
